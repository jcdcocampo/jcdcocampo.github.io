...
(function () {
  ...
  const css = `
    ...
    .cb-chip {
      ...
      animation: cbChipFadeUp 0.5s cubic-bezier(0.22, 1, 0.36, 1) forwards;
    }
    .cb-chip:hover  { background: var(--accent, #007aff); color: #fff; box-shadow: 0 2px 8px rgba(0,122,255,0.22); }
    .cb-chip:active { transform: scale(0.95) translateY(0); }
    .cb-chip.cb-chip-gone { display: none; }
    @keyframes cbChipFadeUp {
      to { opacity: 1; transform: translateY(0); }
    }
    .cb-chip:nth-child(1) { animation-delay: 0.05s; }
    .cb-chip:nth-child(2) { animation-delay: 0.15s; }
    .cb-chip:nth-child(3) { animation-delay: 0.25s; }
  ...
  function createUI() {
    ...
    panel.innerHTML = `
      ...
      <div class="cb-chips" id="cb-chips-row">
        ...
        <button class="cb-chip" type="button">Tell me about his IEEE paper</button>
  ...
  function init() {
    ...
    function openPanel() {
      ...
      if (!welcomeShown) {
        welcomeShown = true;
        // Type the welcome message out for consistency with replies.
        const { bubble } = addBotBubbleForTyping();
        // Tiny delay so the panel finishes its open animation first.
        setTimeout(() => { typeIntoBubble(bubble, CONFIG.welcomeMessage); }, 280);
      }
    }
    function closePanel() {
